use crate::ram::{START_ADDRESS, Ram};
...
impl Cpu {
    ...
    pub fn cycle(&mut self, ram: &mut Ram, screen: &mut Screen) {
        //Fetch
        let op_code = self.fetch(ram);

        //Decode & Execute
        self.decode_and_execute(op_code, ram, screen);
    }

    fn fetch(&mut self, ram: &Ram) -> u16 {
        let op_code = ram.fetch_opcode(self.program_counter as usize);
        self.program_counter += 2;
        op_code
    }

    fn decode_and_execute(&mut self, op_code: u16, ram: &mut Ram, screen: &mut Screen) {
        ...
        match (digit1, digit2, digit3, digit4) {
            ...
            //DRAW
            (0xD, _, _, _) => self.draw_sprite(ram, screen, digit2 as usize, digit3 as usize, digit4 as u8),
            //SKIP KEY PRESS
            ...
            // BCD
            (0xF, _, 3, 3) => self.store_bcd_of_vx_in_ram(ram, digit2 as usize),
            //STORE V0 - VX
            (0xF, _, 5, 5) => self.write_registers_v0_to_vx_in_ram_starting_at_i(ram, digit2 as usize),
            //LOAD V0 - VX
            (0xF, _, 6, 5) => self.read_registers_v0_to_vx_from_ram_starting_at_i(ram, digit2 as usize),
            //should not happen
    ...
    fn draw_sprite(&mut self, ram: &Ram, screen: &mut Screen, vx: usize, vy: usize, sprite_height: u8) {
        ...
        for sprite_row_index in 0..sprite_height {
            ...
            let sprite_pixels_in_row = ram.read_byte(sprite_row_address as usize);
            ...
            for pixel_index in 0..8 {
                if sprite_pixels_in_row & (0b1000_0000 >> pixel_index) != 0 {
                    ...
                    any_pixels_flipped |= screen.get_pixel(screen_index);
                    screen.xor_pixel(screen_index, true);
                }
            }
        }
    ...
    fn store_bcd_of_vx_in_ram(&mut self, ram: &mut Ram, vx: usize) {
        ...
        ram.write_byte(self.i_register as usize, hundreds);
        ram.write_byte(self.i_register as usize + 1, tenths);
        ram.write_byte(self.i_register as usize + 2, ones);
    }

    fn write_registers_v0_to_vx_in_ram_starting_at_i(&mut self, ram: &mut Ram, vx: usize) {
        for i in 0..=vx {
            ram.write_byte(self.i_register as usize + i, self.v_registers[i]);
        }
    }

    fn read_registers_v0_to_vx_from_ram_starting_at_i(&mut self, ram: &Ram, vx: usize) {
        for i in 0..=vx {
            self.v_registers[i] = ram.read_byte(self.i_register as usize + i);
        }
    }
